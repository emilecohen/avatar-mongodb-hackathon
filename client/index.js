...
const avatarID = 'Angela-inblackskirt-20220820';
const voiceID = 'a89e74dd5c094b21bf04291071d84b72';


const statusElement = document.querySelector('#status');
const apiKey = heygen_API.apiKey;
const HEYGEN_URL = heygen_API.serverUrl;
const SERVER_URL = server_API.serverUrl

if (apiKey === 'YourApiKey' || HEYGEN_URL === '') {
...
document.querySelector('#trainBtn').addEventListener('click', trainHandler);


// new session
async function newSession(quality, avatar_name, voice_id) {
  const response = await fetch(`${HEYGEN_URL}/v1/streaming.new`, {
    method: 'POST',
    headers: {
...
async function startSession(session_id, sdp) {
  const response = await fetch(`${HEYGEN_URL}/v1/streaming.start`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Api-Key': apiKey,
...
async function handleICE(session_id, candidate) {
  const response = await fetch(`${HEYGEN_URL}/v1/streaming.ice`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Api-Key': apiKey,
...
async function talkToOpenAI(prompt) {
  ...
  const response = await fetch(`${SERVER_URL}/get-answer`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Access-Control-Allow-Origin': '*',
    },
    body: JSON.stringify({ 'query': prompt, 'company_name': 'mongodb' }),
  });
  if (response.status === 500) {
    console.error('Server error');
    updateStatus(
...
async function generateCompanyKnowledge(url, company_name) {
   const websiteUrl = websiteInput.value;
   const companyName = companyName.value;
   localStorage.setItem('companyName', data.message);

  console.log(JSON.stringify({ url }));
  const response = await fetch(`${SERVER_URL}/index-website`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Access-Control-Allow-Origin': '*',
    },
    body: JSON.stringify({ 'urls': [websiteUrl], 'company_name': companyName}),
  });
  if (response.status === 500) {
    console.error('Server error');
    updateStatus(
...
async function repeat(session_id, text) {
  const response = await fetch(`${HEYGEN_URL}/v1/streaming.task`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Api-Key': apiKey,
...
async function stopSession(session_id) {
  const response = await fetch(`${HEYGEN_URL}/v1/streaming.stop`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-Api-Key': apiKey,
